...
class EleccionesHandler implements IDataBaseHandler
{
    ...
    function getEleccionesPuestosByCondition($idElecciones,$dni,$id_puesto)
    {

        $stm = $this->connection->db->prepare('Select id_puesto,dni FROM Elecciones_cont WHERE id_elecciones = ? and dni = ? and id_puesto = ?');
        $stm->bind_param('isi', $idElecciones, $dni, $id_puesto);
        $stm->execute();
    ...
    function FilterPuesto($dni,$idElecciones,$listaPuestos) {

        foreach($listaPuestos as $key => $list) {
            $filter = $this->getEleccionesPuestosByCondition($idElecciones,$dni,$list->id_puesto);
            if($filter == true) {
    ...
    function AddAuditoria($idElecciones,$id_candidato,$id_partido,$id_puesto,$dni)
    {
        $stm = $this->connection->db->prepare('insert into Elecciones_cont VALUES(?,?,?,?,?)');
        $stm->bind_param('iiiis', $idElecciones,$id_candidato,$id_partido,$id_puesto,$dni);
        $stm->execute();
